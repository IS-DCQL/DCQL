#!/usr/bin/env python3
"""Step 1 -- raw data -> Couchbase document/collection structure (conversion step).

Couchbase uses aggregate document storage, so each logical record collapses into a single
nested JSON document. This script writes the JSON collection files that 2_load.py imports
into Couchbase (one document per row, keyed as described in 2_load.py).
...
and the full polyamide/processing/pa6t json), not the _20 teaching subset. The English attribute
keys produced here are exactly the keys referenced by the conciseness N1QL queries that
3_benchmark.py executes.
...
# Biomedical TCGA raw exports (full cohort, obtained via the GDC link in the paper).
...
# Organic-polymer raw exports (full files, obtained via the NMDMS link in the paper).
